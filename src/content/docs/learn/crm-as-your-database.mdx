...
flowchart TB
  ...
  jobs["scheduled jobs, derived fields<br/>stage dates, display mirrors<br/>diff → PATCH"] <-->|read + write| crm
  crm["CRM — single source of truth"] -->|full list, once a day| dash["static dashboard<br/>metrics computed from the rows"]
```
...
<CourseNav
  ...
  next={{ slug: '/learn/what-an-agent-is', title: 'What an agent actually is' }}
/>
